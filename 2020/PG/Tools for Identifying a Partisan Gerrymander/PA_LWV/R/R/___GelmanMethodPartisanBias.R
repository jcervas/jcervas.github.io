...
pahouse$dvoteimputed <- default.unc(pahouse$VOTE)

house0618no02 <- pahouse[!pahouse$year %in% c(seq(2002,2012,10), 2004, 2018),]
unique.year.0618no02 <- unique(house0618no02$year)
coefs <- array(NA, c(length(unique.year.0618no02 ), 2)) #create empty matrix to store coefficients
  rownames(coefs) <- unique.year.0618no02
resid.errors <- rep(NA, length(unique.year.0618no02)) #empty vector to store residual errors
for (i in 1:length(unique.year.0618no02)){#loop over each 
    fit <- lm(default.unc(VOTE) ~ default.unc(REP_lag), subset = year == unique.year.0618no02[i], data = house0618no02, na.rm=T)
    coefs[i,] <- coef(fit)
...
unique.year.all <- unique(pahouse$year)
...
unique.year.rep <- unique(house.data.replicate$year)


for (i in 1:length(unique.year.all))
{
  y.tmp <- unique.year.all[i]
    adv[i] <- mean(pahouse$dvoteimputed[pahouse$year == y.tmp], na.rm=T)#get average for each year
}
...
for (i in 1:length(unique.year.rep))
...
phi <- 0 #unique(house.data.replicate$incumb.effect[house.data.replicate$year == unique.year.rep[i]]) #get estimated incumbency advantage for particular election year
v.lag <- house.data.replicate$REP_lag[house.data.replicate$year==unique.year.rep[i]]#imputed vote

inc.lag <- rep(0, length(house.data.replicate$REP_lag)) #house.data.replicate$incumb.lag[house.data.replicate$year==unique.year.rep[i]]#lagged incumbency
inc <-  rep(0, length(house.data.replicate$REP_lag)) #house.data.replicate$incumb[house.data.replicate$year==unique.year.rep[i]]#incumbency status
unc <- rep(0, length(house.data.replicate$REP_lag)) #house.data.replicate$uncontested[house.data.replicate$year==unique.year.rep[i]]#uncontested status
...
cbind.data.frame(year=unique.year.rep, predictedseats= predicted.seats, partisanbias=partisan.bias, predictiveerror=predictive.error, predictivesd=predictive.sd)
