...
function computeResult(player,opp){
  ...
  STATES.forEach((s,i)=>{
    ...
    if(winner==="you"){youEC+=EC[i]; youStates.push(s);} else {oppEC+=EC[i]; oppStates.push(s);} });
...
function renderMeansChart(means,player){
  ...
  svg.selectAll(".bar").data(data).enter().append("rect")
    .attr("class","bar").attr("x",d=>x(d.state)).attr("y",d=>y(d.mean))
    .attr("width",x.bandwidth()).attr("height",d=>y(0)-y(d.mean)).attr("fill","#a5b4fc");
  svg.selectAll(".dot").data(data).enter().append("circle")
    .attr("class","dot").attr("cx",d=>x(d.state)+x.bandwidth()/2).attr("cy",d=>y(d.you)).attr("r",5).attr("fill","#111827");
  svg.selectAll(".dot-label").data(data).enter().append("text")
    .attr("x",d=>x(d.state)+x.bandwidth()/2).attr("y",d=>y(d.you)-8)
    .attr("text-anchor","middle").attr("font-size","10px").attr("fill","#111827").text(d=>d.you);
  ...
  svg.append("text").attr("x",W/2).attr("y",M.t).attr("text-anchor","middle").attr("font-weight","600").text("Average vs. Your Allocation");
  svg.append("text").attr("x",W/2).attr("y",M.t+16).attr("text-anchor","middle").attr("fill","#555").attr("font-size","12px").text("Bars: average across all participants; Dots: your choices");
}

function renderResultChart(youEC,oppEC){
  const data=[{name:"You",value:youEC},{name:"Opponent",value:oppEC}];
  const W=400,H=160,M={t:20,r:20,b:30,l:60};
  const svg=d3.select("#pg-result-chart").html("").append("svg").attr("width",W).attr("height",H);
  ...
  svg.selectAll(".bar").data(data).enter().append("rect")
    .attr("x",M.l).attr("y",d=>y(d.name)).attr("width",d=>x(d.value)-M.l).attr("height",y.bandwidth())
    .attr("fill",d=>d.name==="You"?"#111827":"#a5b4fc");
  svg.selectAll(".label").data(data).enter().append("text")
    .attr("x",d=>x(d.value)+5).attr("y",d=>y(d.name)+y.bandwidth()/2).attr("dy",".35em").text(d=>d.value);
...
(function(){
  ...
  btn.addEventListener("click",async()=>{
    ...
    try{
      ...
      const nextStrategyBtn=modal.querySelector(".pg-step--strategy .pg-next");
      ...
      nextStrategyBtn?.addEventListener("click",()=>{
        pgShow("background");
      });
