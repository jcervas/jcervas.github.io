---
title: "Replication Code for Statistical Fallacies in Claims about ‘Massive and Widespread Fraud’ in the 2020 Presidential Election: Examining Claims Based on Aggregate Election Results"
...
**Accepted**, _Statistics and Public Policy_

**In this file, you will find all the code necessary to recreate any table or figures and other data found in "Statistical Fallacies in Claims about ‘Massive and Widespread Fraud’ in the 2020 Presidential Election: Examining Claims Based on Aggregate Election Results".**
...
# Run markdown script
# rmarkdown::render('/Users/cervas/My Drive/GitHub/jcervas.github.io/2023/SPP/readme.Rmd', output_format = 'all')
...
dir.git <- "/Users/cervas/My Drive/GitHub/jcervas.github.io/2023/SPP/data"
dir.online.git <- "https://raw.githubusercontent.com/jcervas/jcervas.github.io/2023/SPP/data"
dir.paper <- "/Users/cervas/My Drive/GitHub/jcervas.github.io/2023/SPP"
dir.data <- paste0(dir.paper, "/data")
dir.figures <- paste0(dir.paper,"/images")
dir.gis <- paste0(dir.paper,"/GIS")
```

```{r, sources, echo=FALSE, include=FALSE}
...
# Source: https://raw.githubusercontent.com/tonmcg/US_County_Level_Election_Results_08-20/master/2016_US_County_Level_Presidential_Results.csv
pres.county.2016 <- safeReadCsv(paste0(dir.git, "/2016_US_County_Level_Presidential_Results.csv"))
...
pres.county.2020 <- safeReadCsv(paste0(dir.git, "/2020_US_County_Level_Presidential_Results.csv"))

# Read Kent County Election Data
kent <- safeReadCsv(paste0(dir.data, "/kent2020.csv"))
```

```{r, pres-2020-county, echo=FALSE}
# Read 2020 Presidential election data by county
## Source: https://observablehq.com/@charliesmart/dorling-cartogram
county.2020 <- safeReadCsv(paste0(dir.data, "/2020_county_results.csv"))
...
if (!is.null(county.2020)) {
  ...
  # Optional: Uncomment to write data
  # write.csv(county.2020, paste0(dir.data, "/county_2020.csv"), row.names=F)
}
```

```{r, GIS-data, echo=FALSE, results = 'hide', warning=FALSE}
# Reading US Census Bureau's County Shapefile
counties.tiger <- rgdal::readOGR(paste0(dir.git, "/tl_2020pl_counties_simplified.json"))

# Reading US Census Bureau's County Cartographic Shapefile
tiger.cart <- rgdal::readOGR(paste0(dir.git, "/cb_2020_us_county_500k_simlified_projected.json"))
```

```{r NYTs-Shapefile, echo=FALSE, results = 'hide', warning=FALSE}
# Reading NYTs County Shapefiles
counties.json <- rgdal::readOGR(paste0(dir.git, "/counties.json"))
state_labels <- rgdal::readOGR(paste0(dir.git, "/state_labels.json"))
states <- rgdal::readOGR(paste0(dir.git, "/states.json"))
state_lines <- rgdal::readOGR(paste0(dir.git, "/statelines.json"))
...
```{r compare-elections, echo=FALSE, include=FALSE}
...
```{r seats-votes, include=FALSE, echo=FALSE}
...
```{r coattails, echo=FALSE, include=FALSE}
...
```{r coattails-plot, echo=FALSE, fig.keep='none'}
...
```{r pres-county-2016, include=FALSE, echo=FALSE}
...
```{r, pres-county-2020, include=FALSE, echo=FALSE}
...
```{r, compare-16-20, eval=F, echo=FALSE}
...
```{r, compare-raw, eval=F, fig.keep='none', echo=FALSE}
...
```{r fig-1, fig.cap="Truth Social post by former President Donald Trump re the lengths he would be prepared to go to overturn the 2020 election results", echo=FALSE, out.width = '100%'}
knitr::include_graphics(paste0("/Users/cervas/My Drive/GitHub/jcervas.github.io/2023/SPP/images/fig-1-1.png"))
```

```{r fig-2, fig.cap="Votes in each County", echo=FALSE}
...
```{r, fig-3, fig.cap="Histogram of the 2020 Presidential Election Results, by County", echo=FALSE}
...
```{r tab-1, echo=FALSE}
# Define the data
types <- c("Arithmetic Fallacies", "Misinterpreting Statistical Significance", 
...
examples <- list(
  c("Cherry-picking the data", "Failing to weight units", 
    "Changes in Support Among Demographic Subgroups", 
    "Two-party vote share versus shares of total votes cast"),
  c("False Causality", "Timing Mirage"),
  c("Double Voting", "Benford’s Law", "Tip of the iceberg fallacy", "Straw man fallacy"),
  c("Spoiled ballots"),
  c("Presidential coattails", "Bellwether counties", "Other Cross-Election Comparisons"),
  c("Split ticket voting versus straight ticket voting", 
    "Comparison of differences involving samples with different means", 
    "Matching design (split ticket voting versus straight ticket voting)", 
    "Matching design (within-election comparisons of areas with and without fraud claims)")
)

# Create a data frame
fallacies_df <- data.frame(Type = types, Examples = I(examples))

# Display the data frame
knitr::kable(fallacies_df, caption = "Statistical Fallacies Evaluated from the 2020 election")
```
...
tab2 <- cbind.data.frame(
  Biden_Counties = rbind.data.frame(
    sum(county.2020.biden$votes_dem),
    sum(county.2020.biden$votes_gop),
    sum(county.2020.biden$diff)
  ),
  Trump_Counties = rbind(
    sum(county.2020.trump$votes_dem),
    sum(county.2020.trump$votes_gop),
    sum(county.2020.trump$diff)
  )
)

# Setting row names
rownames(tab2) <- c("Biden Votes", "Trump Votes", "Difference Pro-Trump")

# Setting column names
colnames(tab2) <- c("Biden Counties", "Trump Counties")

# Table 2 – Popular Vote in Counties Carried by Each Candidate
knitr::kable(tab2, caption = "Popular Vote in Counties Carried by Each Candidate")
...
```{r, GIS-data-combine, echo=FALSE, warning=FALSE, results = 'hide'}
# Combining shapefile data with 2020 election data
## For counties.json
counties.json@data <- dplyr::left_join(counties.json@data, county.2020, by=c("GEOID"))
...
# head(counties.json@data)

# Optional: Filtering out specific states (e.g., Alaska and Hawaii) if needed
# counties.json <- counties.json[!counties.json@data$ST %in% c("AK","HI"),]

# Writing the updated shapefile (uncomment to write)
# rgdal::writeOGR(counties.json, dir.gis, "us2020", driver="ESRI Shapefile", overwrite_layer=TRUE)

# Note: Dissolving states into a national shapefile requires rmapshaper or similar package
nation.json <- rmapshaper::ms_dissolve(states)
```

```{r, exit-polls, echo=FALSE}
...
knitr::kable(rbind(exit.2016,exit.2020), caption = "2016 and 2020 Exit Polls, by Race")
```

```{r, demo-election, echo=FALSE}
...
knitr::kable(tab4, caption = "Change in Non-Hispanic White Votes between 2016 and 2020")
...
counties.json@data$col <- colors.map[findInterval(counties.json@data$per_dem, vec = pop.brks)]

# Calculate absolute margin
absmargin <- abs(counties.json@data$votes_dem - counties.json@data$votes_gop)
...
```{r, fig-4, fig.cap="Choropleth Plot, 2020 Presidential Election by county", echo=FALSE}
# Optional: Uncomment to save the plot as an SVG file
...
sp::plot(counties.json, col=counties.json@data$col, border="#ffffff", lwd=0.15)
sp::plot(states, border="#ffffff", add=TRUE, lwd=1)

# Overlaying national shapefile
sp::plot(nation.json, col=NA, border="#000000", add=TRUE, lwd=1)
...
```{r, fig-5, fig.cap="Bubble Plot, 2020 Presidential Election by county", echo=FALSE}
# Optional: Uncomment to save the plot as an SVG file
...
sp::plot(states, col="#efefef", border="#ffffff", lwd=1)

# Adding transparency to the colors
counties.json@data$col_trans <- ifelse(is.na(counties.json@data$col), counties.json@data$col, paste0(counties.json@data$col, pop.opacity))

# Adding the bubbles
points(counties.json@data$X, counties.json@data$Y, 
       cex=pop.sizes, 
       col="#00000033", 
       bg=counties.json@data$col_trans, 
       pch=21, 
       lwd=1)
...
# Adding legend
circle.r <- scaleSqrt(c(100000,1000000,max(county.2020$total_votes)), maxRadius=5, maxDomain=max(county.2020$total_votes))

# Get the current plot limits
plot_limits <- par("usr")

# Bottom left coordinates (with some offset)
x_coord <- plot_limits[1] + 200000
y_coord <- plot_limits[3] + 200000

# Add a circle
points(x_coord, y_coord, cex=circle.r[1])
points(x_coord, y_coord, cex=circle.r[2])
points(x_coord, y_coord, cex=circle.r[3])
...
counties.json.cart.tmp <- counties.json
counties.json.cart <- counties.json.cart.tmp[!is.na(counties.json.cart.tmp@data$total),]
counties.json.cart@data$margin <- abs(counties.json.cart@data$votes_dem - counties.json.cart@data$votes_gop)

# Creating a non-contiguous cartogram
counties.json.cart1 <- cartogram::cartogram_ncont(counties.json.cart, "margin")

# Creating a contiguous cartogram
counties.json.cart2 <- cartogram::cartogram_cont(counties.json.cart, "margin", itermax=3)
# Optional: Save the cartogram as a shapefile
# rgdal::writeOGR(counties.json, dir.gis, "counties_shp_cart2", driver="ESRI Shapefile", overwrite_layer=TRUE)

# Creating a Dorling cartogram
counties.json.dorling <- cartogram::cartogram_dorling(x=counties.json, weight="margin")
```
...
sp::plot(counties.json.cart1, border="#dddddd", col=counties.json.cart1@data$col, lwd=0.15)
dev.off()

# Plotting and saving contiguous cartogram as SVG
svglite::svglite(paste0(dir.figures,"us2020_cart2.svg"))
sp::plot(counties.json.cart2, border=counties.json.cart2@data$gs.pop.blocks, col=counties.json.cart2@data$col, lwd=0.15)
dev.off()

# Plotting and saving Dorling cartogram as SVG
svglite::svglite(paste0(dir.figures,"us2020_dorling.svg"))
sp::plot(counties.json.dorling, id=counties.json.dorling@data$NAME, border=NA, col=counties.json.dorling@data$col, lwd=0.15)
dev.off()

# Saving non-contiguous and contiguous cartograms as shapefiles
# Optional: Save Cartograms
# rgdal::writeOGR(counties.json.cart1, dir.gis, "counties.json.cart1", driver="ESRI Shapefile", overwrite_layer=TRUE)
# rgdal::writeOGR(counties.json.cart2, dir.gis, "counties.json.cart2", driver="ESRI Shapefile", overwrite_layer=TRUE)
```

```{r, ayy-reg, echo=FALSE}
...
```{r fig-6, fig.cap="Kent County, Michigan 2020 election data plotted as Ayyadurai shows it", echo=FALSE}
# Optional: Uncomment to save the plot as an SVG file
...
```{r fig-7, fig.cap="Kent County, Michigan Precinct comparison between Trump Straight-ticket and Trump Split-Ticket Support", echo=FALSE}
# Optional: Uncomment to save the plot as an SVG file
...
```{r eval=FALSE, include=FALSE, fig.keep='none'}
# Not Used
...
abline(lm(kent$prop_split[sign(kent$biden - kent$trump) >= 0] ~ abs(kent$biden[sign(kent$biden - kent$trump) >= 0] - kent$trump[sign(kent$biden - kent$trump) >= 0])),
    col = "#1375b7BF",
    lwd = 4)
abline(lm(kent$prop_split[sign(kent$biden - kent$trump) <= 0] ~ abs(kent$biden[sign(kent$biden - kent$trump) <= 0] - kent$trump[sign(kent$biden - kent$trump) <= 0])),
    col = "#c93135BF",
